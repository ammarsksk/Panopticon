...
def generate_code_patch_cases(count: int = 500) -> list[CodePatchEvalCase]:
    templates = [
        ...
        (
            ...
            "services/inventory/reservations.py",
            "def reserve_stock(available, requested):\n    return available - requested\n",
            "fix all failing inventory reservation tests",
            ("requested quantity must be positive", "requested quantity exceeds available stock"),
        ),
        (
            "python_shipping_bug_fix",
            "services/shipping/estimate.py",
            "def estimate_delivery_days(country, expedited=False):\n    if expedited:\n        return 2\n    return 5\n",
            "fix all failing shipping estimate tests",
            ("normalized", "return 1 if expedited else 3"),
        ),
        (
            "python_http_robustness",
            "services/platform/http_client.py",
            "def fetch_status(url, client):\n    return client.get(url)\n",
            "make the project more robust by adding network timeouts",
        ...
        (
            ...
            "make config loading more robust with required environment validation",
            ("PAYMENT_URL is required", "raise ValueError"),
        ),
        (
            "python_retry_robustness",
            "services/platform/retry.py",
            "def call_with_retry(fn, attempts=3):\n    return fn()\n",
            "make transient dependency calls more robust with retry handling",
            ("last_error", "for _attempt in range"),
        ),
        (
            "python_validation",
